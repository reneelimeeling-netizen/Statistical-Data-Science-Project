...
;define objects types
    (:types
        rover lander image scan sample location astronaut area - object
    )
...
;list predicates that would be used
    (:predicates
        ...
        ;new predicates for astronauts and areas
        (in ?astr - astronaut ?a - area ?ld - lander)
        (linked ?a1 - area ?a2 - area ?ld - lander)
        (in_docking_bay ?a - area)
        (in_control_room ?a - area)
    )
;list actions that performed by rover, and also parameters, preconditions and effects involved in each action
    (:action move-astronaut
         :parameters (?astr - astronaut ?from - area ?to - area ?ld - lander)
         :precondition (and
             (in ?astr ?from ?ld)
             (linked ?from ?to ?ld)
         )
    ...
    (:action deploy-rover
        :parameters (?r - rover ?ld - lander ?l - location ?astr - astronaut ?a - area)
         :precondition (and
             (lander_at ?ld ?l)
             (at_lander ?r ?ld)
             (not (deployed ?r))
    ...
    (:action store-sample
        :parameters (?r - rover ?s - sample ?ld - lander ?l - location ?astr - astronaut ?a - area)
         :precondition (and
             (deployed ?r)
             (carrying_sample ?r ?s)
             (at ?r ?l)
    ...
    (:action transmit-image
        :parameters (?r - rover ?i - image ?ld - lander ?l - location ?astr - astronaut ?a - area)
        :precondition (and
            (deployed ?r)
            (at ?r ?l)
            (carrying_image ?r ?i)
    ...
    (:action transmit-scan
        :parameters (?r - rover ?sc - scan ?ld - lander ?l - location ?astr - astronaut ?a - area)
        :precondition (and
            (deployed ?r)
            (at ?r ?l)
            (carrying_scan ?r ?sc)
